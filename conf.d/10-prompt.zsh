...
prompt_purification_setup() {
    ...
    ZSH_THEME_GIT_PROMPT_PREFIX="%F{64}(%F{yellow}λ:%f%F{magenta}"
    ZSH_THEME_GIT_PROMPT_SUFFIX="%F{64})%f"
    ZSH_THEME_GIT_PROMPT_DIRTY=""
    ZSH_THEME_GIT_PROMPT_CLEAN=""

    ZSH_THEME_GIT_PROMPT_ADDED="%F{green}+%f "
    ZSH_THEME_GIT_PROMPT_MODIFIED="%F{magenta}%f "
    ZSH_THEME_GIT_PROMPT_DELETED="%F{red}x%f "
    ZSH_THEME_GIT_PROMPT_RENAMED="%F{33}➜%f "
    ...
    prompt_git_branch
    PROMPT=$'%B%F{64}┌─[%f%(#.%F{red}.%F{green})%n%f@%F{$hcolor}%m%F{64}] [%f%~%F{64}] [%F{yellow}%D{%Y-%m-%d %H:%M:%S}%F{64}]%f%b $(prompt_git_info)$(git_prompt_status)
%B%F{64}└─[%(?.%F{green}√.%F{red}%?)%F{64}] %(!.%F{red}#.%F{yellow}$)%f%b '
}
